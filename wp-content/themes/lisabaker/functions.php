...
function remove_rss_shortcodes($content) {
	$content = strip_shortcodes($content);
	return $content;
}

add_filter('the_content_rss', 'remove_rss_shortcodes');
...
if ( ! function_exists( 'boilerplate_continue_reading_link' ) ) :
	...
	function boilerplate_continue_reading_link() {
		return ' <a href="'. get_permalink() . '" class="blogmorebutton">' . __( 'Read More', 'boilerplate' ) . '</a>';
	}
endif;
...
if ( ! function_exists( 'boilerplate_posted_on_date' ) ) :
	/**
	 * Prints HTML with meta information for the current post—date/time and author.
	 *
	 * @since Twenty Ten 1.0
	 */
	function boilerplate_posted_on_date() {
		// BP: slight modification to Twenty Ten function, converting single permalink to multi-archival link
		// Y = 2012
		// F = September
		// m = 01–12
...
if ( ! function_exists( 'boilerplate_posted_cat' ) ) :
	/**
	 * Prints HTML with meta information for the current post (category, tags and permalink).
	 *
	 * @since Twenty Ten 1.0
	 */
	function boilerplate_posted_cat() {
...
/*	Begin Boilerplate */
	...
	function custom_mce_before_init_insert_formats( $init_array ) {  
    		$init_array['theme_advanced_blockformats'] = 'p,h1,h2,h3,h4';
	...
	function create_painting_post_type()
{
    register_post_type('painting',
        ...
        'labels' => array(
            ...
            'view' => __('View Paintings', 'html5blank'),
